...
public class InteractionManager : MonoBehaviour
{
    ...
    [Header("Refs")]
    public PlacementManager placementManager;  // changed: use PlacementManager
    public BallSpawner spawner;      // assign BallSpawner
    public ScoreUI scoreUI;         // assign Score UI
    ...
    public AudioClip placeSound;
    public AudioClip errorSound;
    public AudioClip binHitSound;    // used by ball collision (optional)
    AudioSource audioSource;
    ...
    void UpdateUI()
    {
        if (instructionText != null)
        {
            switch (currentMode)
            {
                ...
                case Mode.PlaceBin: instructionText.text = "Place the Bin (Trigger to place)"; break;
                case Mode.PlaceFan: instructionText.text = "Place the Fan (must be between you and bin)"; break;
                case Mode.Play: instructionText.text = "Throw paper balls! Press 'A' to throw"; break;
            }
        }
        if (placeFanButton != null)
        {
    ...
    // Called by PlacementManager when bin/fan is placed
    public void NotifyPlaced(PlacementManager.Mode mode)
    {
        if (placeSound != null) audioSource.PlayOneShot(placeSound);

        if (mode == PlacementManager.Mode.PlaceBin) binPlaced = true;
        if (mode == PlacementManager.Mode.PlaceFan) fanPlaced = true;
    ...
    // small helper to expose binHitSound for other scripts
    public void PlayBinHitSound(Vector3 pos)
    {
        if (binHitSound != null) AudioSource.PlayClipAtPoint(binHitSound, pos);
    }
}